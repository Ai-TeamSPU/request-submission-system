...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    return res.send(renderRejectForm(id, token, request));
  }

  if (req.method === 'POST') {
    const body = parseFormBody(req.body);
    const { token, managerNote } = body;
    ...
    if (!current) {
      return res.send(renderHtml('ไม่สามารถปฏิเสธได้', 'ไม่พบคำร้องนี้ในระบบ', '#f59e0b'));
    }
    if (current.status !== 'Pending') {
      return res.send(renderHtml('ไม่สามารถปฏิเสธได้', `คำร้องนี้ดำเนินการแล้ว (สถานะ: ${current.status})`, '#f59e0b'));
    }

    const { error } = await supabase
      .from('approvals')
      .update({ status: 'Rejected', manager_note: managerNote || '', updated_at: new Date() })
      .eq('request_id', id);

    if (error) {
      return res.send(renderHtml('ไม่สามารถปฏิเสธได้', error.message, '#f59e0b'));
    }

    notifyTeacherStatusUpdate(id, 'Rejected', managerNote || '').catch(err =>
      console.error('Failed to send status email:', err.message)
    );

    return res.send(renderHtml('ปฏิเสธคำร้องแล้ว', `คำร้อง ${id} ถูกปฏิเสธแล้ว<br>ระบบส่งแจ้งอาจารย์ผู้ยื่นคำร้องเรียบร้อย`, '#dc2626'));
  }
